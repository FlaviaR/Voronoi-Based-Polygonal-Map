...
void assignCornerElevations (void) {
	CenterList::iterator center_iter = centerList.begin();
	
	while (center_iter != centerList.end()) {
		Center* center = *center_iter++;
		
		CornerList::iterator corner_iter = center->corners.begin();
		while (corner_iter != center->corners.end()) {
...
void setOceanBorders (Center* center) {
	CornerList cornerList = center->corners;
	CornerList::iterator ic = cornerList.begin ();

	center->isBorder = false;
	
	while (ic != cornerList.end ()) {
		Corner corner = *ic++;
...
CornerList fetchCorners (Point site) {
...
Center* beginCenterCreation (Point site) {
...
void buildRelationshipLists () {
	...
	while (pl != points.end()) {
		Point p = *pl++;
		Center* c = beginCenterCreation (p);
		
		c->index = index;
		
		centerMap.insert(std::pair<Point, Center*> (c->point, c));
		
		c->neighbors = fetchNeighbors(c->point, centerMap);
